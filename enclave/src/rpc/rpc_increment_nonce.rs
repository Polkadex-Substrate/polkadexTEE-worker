...
impl RpcIncrementNonce {
    ...
    fn method_impl(
        ...
        let nonce = match self
            .rpc_gateway
            .increment_nonce(main_account.clone())
        {
            Ok(b) => Ok(b),
            Err(e) => Err(String::from(e.as_str())),
        }?;
        debug!("Received incremented nonce: {:?}", nonce);

        Ok((nonce, false, DirectRequestStatus::Ok))
    }
}

impl RpcCall for RpcIncrementNonce {
    fn name() -> String {
        "increment_nonce".to_string()
    }
}

impl RpcMethodSync for RpcIncrementNonce {
...
pub mod tests {
    ...
    use crate::rpc::rpc_increment_nonce::RpcIncrementNonce;
    use alloc::boxed::Box;
    use sp_core::Pair;
    use substratee_stf::{Getter, KeyPair, TrustedGetter, TrustedOperation};

    pub fn test_given_valid_top_increment_nonce() {
        let top_extractor = Box::new(TrustedOperationExtractorMock {
            trusted_operation: Some(create_increment_nonce_getter()),
        });

        let rpc_gateway = Box::new(RpcGatewayMock::mock_nonce(true));

        let request = create_dummy_request();
        let rpc_increment_nonce = RpcIncrementNonce::new(top_extractor, rpc_gateway);

        assert!(rpc_increment_nonce.method_impl(request).is_ok())
    }

    fn create_increment_nonce_getter() -> TrustedOperation {
