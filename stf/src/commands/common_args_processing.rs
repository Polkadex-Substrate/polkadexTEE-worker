...
pub fn get_order_from_matches(
	...
	let arg_price = matches.value_of(PRICE_ARG_NAME).map(|v| get_amount_from_str(v));
...
pub fn get_cancel_order_from_matches(
	...
	let order = CancelOrder { user_uid: main_account, market_id, order_id };
...
fn get_market_id_from_matches<'a>(matches: &'a ArgMatches<'a>) -> Result<MarketId, String> {
	...
	Ok(MarketId { base: market_id_base, quote: market_id_quote })
}
